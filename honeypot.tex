...
\begin{abstract}
  ...
  \keywords{Cybersecurity \middot\ Network Security \middot\ Honeypot 
            \middot\ Teaching} 
\end{abstract}
...
together with sandboxed deployment of LIHV and HILV honeypots provides 
a stimulating, cost-effective environment for the cybersecurity studies
...
in the specialist teaching laboratories. The complete HILV honeypot
consists of:
...
The two additional devices shown in Figure~\ref{fig:HPOverview} could be PCs to
act as attack entry points or target devices such as PCs or wireless access
points. These devices could also be PCs supporting virtualisation to extend the
honeypot's functionality. The approximate cost of the basic \texttt{HILV},
without additional devices or the monitoring PC attached, is £450. This offers
a highly stimulating environment for the teaching of practical cybersecurity
skills at a very low cost.
...
Using Raspberry Pi boards as the main server technology within the honeypot allows 
reconfiguration and rebuilding of the architecture to be simplified.
...
be restored. Multiple configurations can be kept on different image sets and
...
\texttt{IP} address from the Internet-based router to the HILV honeypot
...
circuits. A virtual circuit is an internal path whereby traffic is filtered by
the \texttt{Ethernet\_II} frame header. Switches maintain an in-memory table of
ports and \texttt{MAC} addresses to pass each packet to a specific port. The
result of this routing is that packets are transferred port to port and are not
broadcast, which limits packet capture. When using a honeypot, packet capture is
a vital part of the architecture for the analysis of any network-based attack
...
A further issue with the capture of the honeypot network traffic 
is the possibility of packet loss on the port where the mirrored traffic is forwarded.
Switches are designed to provide maximum transfer speed 
